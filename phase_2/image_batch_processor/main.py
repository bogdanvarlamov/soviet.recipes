...
def main():
    """Execute the batch processing flow with Docling engine."""
    # Setup logging
    setup_logging()
    logger = logging.getLogger(__name__)
    
    logger.info("Starting image batch processor with Docling engine")
    
    # Define paths (relative to workspace root)
    image_dir = Path("../../phase_1/cookbook_images")
    output_dir = Path("./output/docling_extraction")
    
    # Create initial state with Docling engine configuration
